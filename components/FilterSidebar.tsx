...
export const FilterSidebar: React.FC<FilterSidebarProps> = ({ onOpenSearchService }) => {
  return (
    <div className="bg-white p-8 border border-gray-100 sticky top-32 shadow-soft">
      <div className="flex justify-between items-center mb-8 border-b border-greige pb-4">
        ...
        <button className="text-[10px] uppercase tracking-widest text-gray-400 hover:text-navy transition-colors">Rensa allt</button>
      </div>
      ...
      <div className="space-y-10">
        ...
        {/* Region */}
        <div>
          <label className="text-[10px] uppercase tracking-[0.2em] text-sage font-bold mb-4 block">Region</label>
          <div className="space-y-3">
            {['Costa del Sol', 'Costa Blanca'].map(region => (
              <label key={region} className="flex items-center gap-3 cursor-pointer group">
                <div className="relative flex items-center">
                  <input type="checkbox" className="peer appearance-none w-4 h-4 border border-gray-300 rounded-sm checked:bg-navy checked:border-navy transition-colors" />
                ...
                <span className="text-sm font-light text-charcoal group-hover:text-navy transition-colors">{region}</span>
              </label>
            ))}
          </div>
        </div>
        ...
        {/* Price Range */}
        <div>
          <label className="text-[10px] uppercase tracking-[0.2em] text-sage font-bold mb-4 block">Pris (€)</label>
        ...
        {/* Bedrooms */}
        <div>
          <label className="text-[10px] uppercase tracking-[0.2em] text-sage font-bold mb-4 block">Sovrum</label>
        ...
        {/* Features */}
        <div>
          <label className="text-[10px] uppercase tracking-[0.2em] text-sage font-bold mb-4 block">Egenskaper</label>
          <div className="space-y-3">
            {['Havsutsikt', 'Pool', 'Nyproduktion', 'Terrass', 'Turistlicens'].map(feature => (
              <label key={feature} className="flex items-center gap-3 cursor-pointer group">
                ...
                <span className="text-sm font-light text-charcoal group-hover:text-navy transition-colors">{feature}</span>
              </label>
            ))}
          </div>
        </div>
        ...
        {/* Concierge Helper Card - Now Connected */}
        {onOpenSearchService && (
          <div className="mt-8 pt-8 border-t border-gray-100">
             <div className="bg-navy p-6 text-center rounded-sm relative overflow-hidden group cursor-pointer" onClick={onOpenSearchService}>
                <div className="absolute top-0 right-0 w-24 h-24 bg-white/5 rounded-full blur-2xl translate-x-1/2 -translate-y-1/2"></div>
                <h4 className="font-serif text-white text-lg mb-2">Hittar du inte rätt?</h4>
                <p className="text-white/60 text-xs font-light mb-4">Låt oss söka åt dig kostnadsfritt.</p>
