...
import { Search, MapPin, ChevronDown, Home, Palmtree, Waves, Sun, Building2, Coins, Plus, Minus, Check } from 'lucide-react';
...
export const Hero: React.FC = () => {
  ...
  const selectOption = (type: 'location' | 'type' | 'price', value: string) => {
  ...
  return (
    <div className="relative min-h-screen md:h-[95vh] w-full flex flex-col justify-start md:justify-center items-center bg-navy overflow-hidden pt-32 md:pt-0">
      ...
      <div className="absolute inset-0 z-0">
        ...
        <div className="absolute inset-0 bg-gradient-to-t from-navy/90 via-navy/20 to-navy/40"></div>
      </div>
      ...
      <div className="relative z-10 w-full max-w-7xl px-6 flex flex-col items-center">
        ...
        <h1 className="text-5xl md:text-7xl lg:text-8xl font-serif text-white mb-6 leading-[1.05] text-center drop-shadow-lg tracking-tight">
          Hitta hemmet där <br /> 
          <span className="italic font-light text-alabaster/90">hjärtat landar</span>
        </h1>
        ...
        <p className="text-lg md:text-xl text-white/90 mb-12 max-w-2xl mx-auto font-light leading-relaxed text-center antialiased opacity-90">
          Vi guidar dig genom Costa Blancas och Costa del Sols mest eftertraktade områden. Från första visning till första doppet i poolen.
        </p>

        {/* --- SEARCH INTERFACE --- */}
        <div ref={searchContainerRef} className="w-full max-w-5xl mx-auto bg-white rounded-md shadow-2xl transition-all duration-300 relative">
          ...
          {/* Main Search Layout - Stacked on Mobile, Grid on Desktop */}
          <div className="flex flex-col md:grid md:grid-cols-12 gap-0 divide-y md:divide-y-0 md:divide-x divide-gray-100 p-2 relative">
            ...
            >
              <label className="text-[10px] uppercase tracking-[0.2em] text-sage font-bold mb-1 block group-hover:text-navy transition-colors">Var vill du bo?</label>
              <div className="flex items-center justify-between">
                <div className="text-navy font-serif text-lg truncate pr-2">
                  {selectedRegion || 'Välj område'}
                </div>
                <MapPin size={18} className="text-gray-300 group-hover:text-sand transition-colors" />
              </div>
              <p className="text-xs text-gray-400 mt-1 font-light truncate">Costa Blanca & Costa del Sol</p>
              ...
              {openDropdown === 'location' && (
                <div className="absolute top-full left-0 w-full bg-white shadow-xl border border-gray-100 rounded-b-sm z-50 animate-fade-in mt-2 md:mt-0">
                  <div className="p-2 space-y-1">
                    {['Hela Spanien', 'Costa del Sol', 'Costa Blanca'].map((loc) => (
                      <button 
                        key={loc}
                        onClick={(e) => { e.stopPropagation(); selectOption('location', loc); }}
                      ...
                      >
                        {loc}
                        {selectedRegion === loc && <Check size={14} className="text-sage" />}
                      </button>
                    ))}
                  </div>
                </div>
            ...
            >
              <label className="text-[10px] uppercase tracking-[0.2em] text-sage font-bold mb-1 block group-hover:text-navy transition-colors">Typ av bostad</label>
              <div className="flex items-center justify-between">
                <div className="text-navy font-serif text-lg">{selectedType}</div>
                <ChevronDown size={18} className="text-gray-300 group-hover:text-navy transition-colors" />
              </div>
              <p className="text-xs text-gray-400 mt-1 font-light">Villa, Lägenhet, Takvåning...</p>

              {/* Type Dropdown */}
              {openDropdown === 'type' && (
                <div className="absolute top-full left-0 w-full bg-white shadow-xl border border-gray-100 rounded-b-sm z-50 animate-fade-in mt-2 md:mt-0">
                  <div className="p-2 space-y-1">
                    {['Alla typer', 'Villa', 'Lägenhet', 'Radhus', 'Takvåning'].map((type) => (
                      <button 
                        key={type}
                        onClick={(e) => { e.stopPropagation(); selectOption('type', type); }}
                        className="w-full text-left px-4 py-3 hover:bg-greige/30 text-navy font-medium text-sm rounded-sm flex justify-between items-center"
                      >
                        {type}
                        {selectedType === type && <Check size={14} className="text-sage" />}
                      </button>
                    ))}
                  </div>
                </div>
            ...
            >
              <label className="text-[10px] uppercase tracking-[0.2em] text-sage font-bold mb-1 block group-hover:text-navy transition-colors">Din budget</label>
              <div className="flex items-center justify-between">
                <div className="text-navy font-serif text-lg">{selectedPrice}</div>
                <Coins size={18} className="text-gray-300 group-hover:text-sand transition-colors" />
              </div>
              <p className="text-xs text-gray-400 mt-1 font-light">Maxpris i Euro (€)</p>

              {/* Price Dropdown */}
              {openDropdown === 'price' && (
                <div className="absolute top-full left-0 w-full bg-white shadow-xl border border-gray-100 rounded-b-sm z-50 animate-fade-in mt-2 md:mt-0">
                  <div className="p-2 space-y-1 max-h-60 overflow-y-auto">
                    {['Valfritt pris', '€ 150 000', '€ 250 000', '€ 350 000', '€ 500 000', '€ 750 000', '€ 1 000 000+'].map((price) => (
                      <button 
                        key={price}
                        onClick={(e) => { e.stopPropagation(); selectOption('price', price); }}
                        className="w-full text-left px-4 py-3 hover:bg-greige/30 text-navy font-medium text-sm rounded-sm flex justify-between items-center"
                      >
                        {price}
                        {selectedPrice === price && <Check size={14} className="text-sage" />}
                      </button>
                    ))}
                  </div>
                </div>
            ...
            <div className="md:col-span-2 p-2">
              <button className="w-full h-14 md:h-full bg-navy text-white rounded-sm hover:bg-charcoal transition-all duration-300 flex flex-row md:flex-col items-center justify-center gap-2 shadow-lg group">
                <Search size={20} className="group-hover:scale-110 transition-transform" />
                <span className="uppercase tracking-[0.15em] text-xs font-semibold">Sök</span>
              </button>
            </div>
          </div>
          ...
          <div className="border-t border-gray-100 bg-gray-50/50 px-4 md:px-6 py-3 flex flex-col md:flex-row justify-between items-center gap-4">
             <div className="flex items-center gap-4 overflow-x-auto w-full md:w-auto pb-2 md:pb-0 scrollbar-hide mask-fade-right">
                <span className="text-[10px] uppercase tracking-widest text-gray-400 font-semibold whitespace-nowrap hidden md:block">Snabba val:</span>
                
                <label className="flex items-center gap-2 cursor-pointer group flex-shrink-0">
                  <div className="w-4 h-4 rounded-sm border border-gray-300 bg-white flex items-center justify-center group-hover:border-navy transition-colors"></div>
          ...
          {/* Expanded Advanced Filters Area (Collapsible) */}
          <div className={`bg-white border-t border-gray-100 overflow-hidden transition-all duration-500 ease-in-out ${isAdvancedOpen ? 'max-h-[600px] opacity-100' : 'max-h-0 opacity-0'}`}>
            <div className="p-6 md:p-8 grid grid-cols-1 md:grid-cols-4 gap-6 md:gap-8">
                <div>
                    <h4 className="text-xs font-bold uppercase tracking-widest text-sage mb-4">Rum & Yta</h4>
                    <div className="flex gap-2 mb-4">
                        {[1, 2, 3, 4, '5+'].map(n => (
                            <button key={n} className="w-8 h-8 rounded-full border border-gray-200 text-xs font-medium text-gray-500 hover:border-navy hover:text-navy hover:bg-navy/5 transition-all">{n}</button>
                        ))}
                    </div>
                    <div className="text-xs text-gray-400">Antal sovrum</div>
                </div>
                <div>
                    <h4 className="text-xs font-bold uppercase tracking-widest text-sage mb-4">Faciliteter</h4>
                    <div className="space-y-2">
                        {['Garage / Parkering', 'Hiss', 'AC', 'Golfnära', 'Gångavstånd strand'].map(opt => (
                            <label key={opt} className="flex items-center gap-2 text-sm text-gray-600 cursor-pointer hover:text-navy">
                ...
                <div>
                    <h4 className="text-xs font-bold uppercase tracking-widest text-sage mb-4">Område specificitet</h4>
                    <div className="space-y-2">
                        {['Frontline Beach', 'Gated Community', 'Bergsby', 'Stadspuls'].map(opt => (
                            <label key={opt} className="flex items-center gap-2 text-sm text-gray-600 cursor-pointer hover:text-navy">
                                <div className="w-4 h-4 border border-gray-300 rounded-sm"></div>
                                {opt}
                            </label>
        ...
        <div className="mt-12 flex flex-wrap justify-center gap-4 md:gap-8 text-white/70 text-[9px] md:text-[10px] tracking-[0.2em] uppercase font-medium animate-fade-in text-center pb-8 md:pb-0">
          <div className="flex items-center gap-2 md:gap-3">
            <span className="w-1.5 h-1.5 md:w-2 md:h-2 border border-sand rounded-full"></span>
