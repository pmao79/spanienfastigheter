...
export const sendMailing = action({
    ...
    handler: async (ctx, { mailingId }) => {
        ...
        let fromAddress = "Spanienfastigheter <info@spanienfastigheter.se>";
        let replyToAddress = senderEmail;

        if (senderEmail && senderEmail.endsWith(COMPANY_DOMAIN)) {
            // Case A: Internal Agent with Verified Domain
            fromAddress = `${senderName} <${senderEmail}>`;
        } else {
            // Case B: External/Partner Agent
            // fromAddress remains generic info@...
            // replyToAddress ensures customer replies go to the agent
        }
