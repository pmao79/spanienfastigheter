    ...
    useEffect(() => {
        if (lead) {
            setEditForm({
                ...
                source: lead.source || "",
                notes: lead.notes || "",
                temperature: (lead.temperature as any) || "cold",
            });
        }
    }, [lead]);
                            ...
                            <div className="rounded-xl bg-white p-6 shadow-sm">
                                ...
                                <div className="grid grid-cols-2 gap-4 text-sm">
                                    <div className="col-span-2 sm:col-span-1">
                                        <label className="block text-slate-500">Namn</label>
                                        <span className="font-medium text-slate-900">{lead.firstName} {lead.lastName}</span>
                                    </div>
                                    <div className="col-span-2 sm:col-span-1">
                                    ...
                                    <div className="col-span-2 sm:col-span-1">
                                        <label className="block text-slate-500">Telefon</label>
                                        <span className="font-medium text-slate-900">{lead.phone || "-"}</span>
                                    </div>
                                    <div className="col-span-2 sm:col-span-1">
                                        <label className="block text-slate-500">Källa</label>
                                    ...
                                    <div className="col-span-2">
                                        ...
                                        <p className="text-slate-900 whitespace-pre-line">{lead.notes || "Inga anteckningar"}</p>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <form onSubmit={handleUpdateLead} className="space-y-4">
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <input type="text" value={editForm.firstName} onChange={e => setEditForm(prev => ({ ...prev, firstName: e.target.value }))} className="mt-1 block w-full rounded-md border-gray-300 border p-2 text-sm" />
                                </div>
                                <div>
                                    <label className="block text-sm font-medium text-slate-700">Efternamn</label>
                                    <input type="text" value={editForm.lastName} onChange={e => setEditForm(prev => ({ ...prev, lastName: e.target.value }))} className="mt-1 block w-full rounded-md border-gray-300 border p-2 text-sm" />
                                </div>
                            </div>
                            <div>
                                <label className="block text-sm font-medium text-slate-700">Email</label>
                                <input type="email" value={editForm.email} onChange={e => setEditForm(prev => ({ ...prev, email: e.target.value }))} className="mt-1 block w-full rounded-md border-gray-300 border p-2 text-sm" />
                            </div>
                            <div>
                                <label className="block text-sm font-medium text-slate-700">Telefon</label>
                                <input type="text" value={editForm.phone} onChange={e => setEditForm(prev => ({ ...prev, phone: e.target.value }))} className="mt-1 block w-full rounded-md border-gray-300 border p-2 text-sm" />
                            </div>
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    <label className="block text-sm font-medium text-slate-700">Källa</label>
                                    <input type="text" value={editForm.source} onChange={e => setEditForm(prev => ({ ...prev, source: e.target.value }))} className="mt-1 block w-full rounded-md border-gray-300 border p-2 text-sm" />
                                </div>
                                <div>
                                    <label className="block text-sm font-medium text-slate-700">Temperatur</label>
                                    <select value={editForm.temperature} onChange={e => setEditForm(prev => ({ ...prev, temperature: e.target.value as any }))} className="mt-1 block w-full rounded-md border-gray-300 border p-2 text-sm bg-white">
                            ...
                            <div>
                                <label className="block text-sm font-medium text-slate-700">Anteckningar (Översikt)</label>
                                <textarea value={editForm.notes} onChange={e => setEditForm(prev => ({ ...prev, notes: e.target.value }))} className="mt-1 block w-full rounded-md border-gray-300 border p-2 text-sm" rows={4} />
                            </div>

                            <div className="mt-6 flex justify-end gap-3">
                                <button type="button" onClick={() => setIsEditModalOpen(false)} className="rounded-lg px-4 py-2 text-sm text-slate-600 hover:bg-slate-100">Avbryt</button>
                                <button type="submit" className="rounded-lg bg-[#1a365d] px-4 py-2 text-sm font-medium text-white hover:bg-[#1a365d]/90">Spara ändringar</button>
                            </div>
                        </form>
                    </div>
                </div>
